...
function uploadpengaturan()
{
    $namafile = $_FILES['logo']['name'] ?? '';
    $ukuran   = $_FILES['logo']['size'] ?? 0;
    $error    = $_FILES['logo']['error'] ?? 4;
    $tmp_name = $_FILES['logo']['tmp_name'] ?? '';

    // Jika tidak ada file yang diupload
    if ($error === 4) {
        return false; // akan gunakan logo lama atau default
    }

    // Validasi ukuran
    if ($ukuran > 2000000) {
        echo "<script>alert('Ukuran logo terlalu besar (maks. 2MB)'); window.location.href='?page=pengaturan';</script>";
        return false;
    }
    ...
    if (!in_array($ekstensi, $ekstensivalid)) {
        echo "<script>alert('File yang diupload harus gambar!'); window.location.href='?page=pengaturan';</script>";
        return false;
    }
    ...
    if (!move_uploaded_file($tmp_name, $tujuan)) {
        echo "<script>alert('Gagal mengupload logo!'); window.location.href='?page=pengaturan';</script>";
        return false;
    }
...
function ubahpengaturan($data)
{
    global $conn;

    $id         = $data['id_pengaturan'];
    $appname    = htmlspecialchars($data['appname']);
    $copyright  = htmlspecialchars($data['copyright']);
    $description = htmlspecialchars($data['description']);
    $keywords   = htmlspecialchars($data['keywords']);
    $author     = htmlspecialchars($data['author']);
    $logolama   = $data['logolama'] ?? 'default.png';

    // Cek apakah user upload logo baru
    if ($_FILES['logo']['error'] === 4) {
        $logo = $logolama; // pakai logo lama
    } else {
        $logo = uploadpengaturan();
        if ($logo) {
            // Hapus logo lama jika ada dan bukan default
            $pathlama = "assets/uploads/" . $logolama;
            if (file_exists($pathlama) && $logolama != 'default.png') {
                unlink($pathlama);
            }
        } else {
            // Jika upload gagal, batalkan proses
            return false;
        }
    }
